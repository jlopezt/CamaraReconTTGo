...
void WiFiManager::handleRoot() {
  ...
  String page = FPSTR(HTTP__HEAD);
  page.replace("{v}", "Options");
  page += FPSTR(HTTP__SCRIPT);
  page += FPSTR(HTTP__STYLE);
  ...
  page += FPSTR(HTTP__PORTAL_OPTIONS);
  page += FPSTR(HTTP__END);
...
void WiFiManager::handleWifi(boolean scan) {
  ...
  for (int i = 0; i < _paramsCount; i++) {
    ...
    String pitem = FPSTR(HTTP__FORM_PARAM);
  ...
  if (_sta_static_ip) {
    ...
    String item = FPSTR(HTTP__FORM_PARAM);
    item.replace("{i}", "ip");
    item.replace("{n}", "ip");
  ...
  page += FPSTR(HTTP__FORM_END);
  page += FPSTR(HTTP__SCAN_LINK);
